...
fn main() -> Result<(), header_completer::error::Error> {
    ...
    let database: header_completer::compilation_database::CompilationDatabase =
        serde_json::from_reader(reader).map_err(|e| format!("failed to load database: {}", e))?;
    let database = header_completer::complete_compilation_database(&database, cli.pattern)?;
